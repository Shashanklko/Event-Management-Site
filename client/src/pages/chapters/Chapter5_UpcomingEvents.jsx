            ...
            return (
              <FadeEvent key={event.id}>
                ...
                <div className="relative z-10 w-full flex flex-col items-center">
                  ...
                  <h2 className="text-5xl md:text-7xl lg:text-[6rem] font-serif-luxury text-white tracking-widest leading-tight mb-8 group-hover:text-amber-200 transition-colors duration-700 max-w-5xl">
                    {event.name}
                  </h2>
                  ...
                  <div className="flex flex-col md:flex-row items-center gap-6 md:gap-12 opacity-60 group-hover:opacity-100 transition-opacity duration-700">
                    <span className="text-xs tracking-[0.3em] uppercase text-amber-400 font-mono">
                      {event.venue}
                    </span>
                    <div className="w-12 h-[1px] md:w-[1px] md:h-6 bg-white/20" />
                    <span className="text-xs tracking-[0.3em] uppercase text-amber-400 font-mono">
                      {date.toLocaleString('en-US', { month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                    </span>
                  </div>
                  ...
                  <div className="mt-12 opacity-0 group-hover:opacity-100 transition-all duration-700 transform translate-y-4 group-hover:translate-y-0">
                    <button 
                      ...
                      className="px-8 py-3 rounded-full border border-amber-500/30 text-[10px] tracking-widest font-mono text-amber-400 uppercase hover:bg-amber-500/10 transition-colors duration-300 flex items-center gap-4"
                    >
